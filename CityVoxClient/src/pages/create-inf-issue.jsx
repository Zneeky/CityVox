...
import { CreateInfIssue as CreateInfIssueAPICall } from "../utils/api";
import { InfIssueTypes } from "../utils/consts";
import { uploadToCloudinary } from "../utils/api";

// Main component for creating an infrastructure issue request
const CreateInfIssue = () => {
  ...
  // State to manage the form data for creating an infrastructure issue request
  ...
  // Function to handle the form submission for creating an infrastructure issue request
  const handleFormSubmit = async (formData) => {
    ...
    // Make API call to create an infrastructure issue request with the form data
    const res = await CreateInfIssueAPICall(appUser.accessToken, formData);

    // Show an alert with the infrastructure issue request submission confirmation
    alert(
      `Your infrastructure issue has been submitted for investigation! ID:${res}`
    );
  ...
  // JSX structure for rendering the CreateInfIssue component
  return (
    <>
      {/* Top navigation bar */}
      ...
      <LayoutRoot>
        <LayoutContainer>
          ...
          {/* Title for the Create Infrastructure Issue page */}
          <Typography p={"1.6rem"} variant="h5">
            Create a request about an infrastructure issue
          </Typography>
...
// Export the CreateInfIssue component as the default export
export default CreateInfIssue;
