...
import { GetApprovedIssuesForUser, uploadToCloudinary, CreatePost as CreatePostApi, CreateFormalPost as CreateFormalPostApi } from '../../utils/api';
...
const CreatePost = ({ open, handleClose }) => {
    ...
    const handlePostSubmit = async (values) => {
        if (user.role !== "representative") {
            ...
            if (values.images && values.images.length > 0) {
                const uploadPromises = values.images.map(file => uploadToCloudinary(file));
            ...
            const [selectedIssueId, selectedIssueType] = values.selectedIssue.split(',');
    ...
    return (
        <Dialog
            ...
            PaperProps={{ // This is used to style the inner paper of the dialog
                style: {
                            ...
                            <WidgetWrapper>
                                    ...
                                    (userIssues.length > 0 ? (
                                        <Box mb={2}>
                                            <Select
                                                ...
                                                onChange={(event) => setFieldValue("selectedIssue", event.target.value)}
                                                fullWidth
                                                variant="outlined"
                                            >
                                                {userIssues.map((issue, index) => {
                                                    return (
                                                        <MenuItem key={index} value={`${issue.Id},${getNumber(issue.Represent)}`}>
                                                            {issue.Title} - ID:{issue.Id} - Muni:{issue.Municipality}
                                                        </MenuItem>
                                                    );
                                                })}
                                            </Select>
                                        ...
                                        <Box mb={2} display="flex" justifyContent="center">
                                            <Typography color="red" variant='body2'>You do not have any viable issues to post about</Typography>
                                        </Box>
                                    ))
                                }
                                {isImage && (
                                ...
                                <FlexBetween>
                                    <FlexBetween gap="0.25rem" onClick={() => setIsImage(!isImage)}>
                                        ...
                                        <Typography
                                            color={palette.primary.main}
                                            sx={{ "&:hover": { cursor: "pointer", color: palette.primary.medium } }}
                                        >
                                            Image
                                        </Typography>
                                    </FlexBetween>
